...
/**
 * ECS system for protecting blocks from unauthorized placement.
 *
 * Prevents players from placing blocks in claimed chunks unless they own
 ...
 * - Can cancel the event to prevent the block placement
 */
class BlockPlaceProtectionSystem(
    ...
    /**
     * Handle a block place event and prevent it if unauthorized.
     *
    ...
    override fun handle(
        ...
        if (!claimsService.canBuild(playerId, chunkPosition)) {
            ...
            LoggingManager.debug(logger, "claims") {
                "Blocked block place by $playerId at chunk ($chunkX, $chunkZ)"
            }
        }
    }
}
