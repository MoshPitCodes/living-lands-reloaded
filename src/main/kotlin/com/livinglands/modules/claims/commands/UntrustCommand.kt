...
import com.livinglands.modules.claims.UntrustPlayerResult
import java.awt.Color
import java.util.UUID

/**
 * Command to untrust a player in the current claim.
 *
 * Usage: /ll untrust <player>
 *
 * Revokes build permission for the specified player in your claim at the current location.
 */
class UntrustCommand : AsyncModuleCommand(
    name = "untrust",
    description = "Revoke a player's build permission",
    moduleId = "claims",
    operatorOnly = false
) {
    ...
    override fun validateInputs(ctx: CommandContext): ValidationResult {
        ...
        if (args.isEmpty()) {
            return ValidationResult.error("Usage: /ll untrust <player>")
        }
    ...
    override suspend fun executeAsyncIfModuleEnabled(ctx: CommandContext, validatedData: Any?) {
        ...
        // Untrust the player
        val result = service.untrustPlayer(playerId, claim.id, targetPlayerId)

        when (result) {
            is UntrustPlayerResult.Success -> {
                MessageFormatter.commandRaw(ctx, "✓ Untrusted $targetPlayerName from this claim", green)
            }
            is UntrustPlayerResult.ClaimNotFound -> {
                MessageFormatter.commandError(ctx, "Claim not found")
            }
            is UntrustPlayerResult.NotOwner -> {
                MessageFormatter.commandError(ctx, "You don't own this claim")
            }
            is UntrustPlayerResult.NotTrusted -> {
                MessageFormatter.commandError(ctx, "$targetPlayerName is not trusted in this claim")
            }
            is UntrustPlayerResult.DatabaseError -> {
                MessageFormatter.commandError(ctx, "Failed to save changes - contact server admin")
            }
        }
    }
