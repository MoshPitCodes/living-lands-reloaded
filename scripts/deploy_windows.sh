...
if [ ! -f "build/libs/livinglands-reloaded-1.1.0.jar" ]; then
	echo "ERROR: Main jar not found. Run './gradlew build' first."
	exit 1
fi

# Remove old jars with different names (if exist)
if [ -f "$GLOBAL_MODS_DIR/livinglands-1.1.0.jar" ]; then
	echo "Removing old jar: livinglands-1.1.0.jar"
	rm "$GLOBAL_MODS_DIR/livinglands-1.1.0.jar"
fi
if [ -f "$GLOBAL_MODS_DIR/livinglands-reloaded-1.1.0-shadow.jar" ]; then
	echo "Removing old jar: livinglands-reloaded-1.1.0-shadow.jar"
	rm "$GLOBAL_MODS_DIR/livinglands-reloaded-1.1.0-shadow.jar"
fi

# Copy main jar to mods directory
echo "Copying JAR to mods directory..."
cp build/libs/livinglands-reloaded-1.1.0.jar "$GLOBAL_MODS_DIR/livinglands-reloaded-1.1.0.jar"

# Verify copy
JAR_SIZE=$(stat -c%s "$GLOBAL_MODS_DIR/livinglands-reloaded-1.1.0.jar")
echo "Deployed: livinglands-reloaded-1.1.0.jar ($((JAR_SIZE / 1024 / 1024)) MB)"
...
unzip -o "build/libs/livinglands-reloaded-1.1.0.jar" "Common/*" -d "$ASSET_PACK_DIR" 2>/dev/null || true
...
echo ""
echo "Deployed files:"
echo "  1. JAR: $GLOBAL_MODS_DIR/livinglands-reloaded-1.1.0.jar"
